...
async def run_graph_feature_pipeline(app_config, exchange):
    ...
    try:
        ...
        # --- THIS IS THE FIX ---
        if gnn_exchange:
            await gnn_exchange.close() # Corrected typo from gn_exchange
        # --- END OF FIX ---
...
async def get_aligned_mtf_data(symbol: str, app_config, exchange):
    ...
    if df_tactical_raw.empty or df_strategic_raw.empty or df_micro_raw.empty:
        print(f"[DataProcessing] ERROR: Raw data missing for {symbol}. Tactical: {df_tactical_raw.empty}, Strategic: {df_strategic_raw.empty}, Micro: {df_micro_raw.empty}")
        return pd.DataFrame()

    # --- START OF THE FIX ---

    # 1. Process all features for all TFs first
    df_tactical_processed = add_all_features(df_tactical_raw.copy())
    df_strategic_processed = add_all_features(df_strategic_raw.copy())
    tfi_series = get_trade_flow_imbalance(df_micro_raw)

    # 2. Find the exact 'ATRr' column name (e.g., 'ATRr_14')
    # This is CRITICAL for the backtester
    tactical_atr_col = next((c for c in df_tactical_processed.columns if c.startswith('ATRr')), None)

    if tactical_atr_col is None:
        print(f"[DataProcessing] ERROR in get_aligned_mtf_data: 'ATRr' column not found in tactical data for {symbol}")
        return pd.DataFrame()  # Fail fast if ATRr is missing
    
    # 3. Create the base 'aligned_df'
    # This MUST contain the NON-PREFIXED columns required by the backtester.
    # These columns will be passed in 'X_val' to the fitness function.
    base_cols_to_keep = ['open', 'high', 'low', 'close', tactical_atr_col]
    # Ensure we only keep columns that actually exist
    existing_base_cols = [col for col in base_cols_to_keep if col in df_tactical_processed.columns]

    if len(existing_base_cols) != 5:
         print(f"[DataProcessing] ERROR: Base columns missing. Found: {existing_base_cols}")
         return pd.DataFrame()

    aligned_df = df_tactical_processed[existing_base_cols].copy()

    # 4. Create prefixed feature dataframes for the model
    # These are the columns the model will actually see for training/prediction.
    df_tactical_features = df_tactical_processed.add_prefix('tactical_')
    df_strategic_features = df_strategic_processed.add_prefix('strategic_')

    # 5. Reset indexes on all dataframes for 'merge_asof'
    ...
    # 6. Merge all feature dataframes onto the base dataframe
    # We merge the prefixed tactical features
    aligned_df = pd.merge_asof(aligned_df, df_tactical_features, on='timestamp', direction='backward')
    # We merge the prefixed strategic features
    aligned_df = pd.merge_asof(aligned_df, df_strategic_features, on='timestamp', direction='backward')
    # We merge the microstructure features (tfi)
