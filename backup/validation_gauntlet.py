...
import random
from forge.crucible.numba_backtester import NumbaTurboBacktester as Backtester
...
def run_in_sample_backtest(model, df_train: pd.DataFrame, features: list) -> dict:
    """
    ...
    try:
        ...
        results = backtester.run_backtest(df_train, predictions)
        
        sharpe = results['metrics'].get('sharpe_ratio', 0)
        pnl = results['metrics'].get('total_pnl_pct', 0)
...
def run_walk_forward_analysis(model_class_name, model_params, df_train, features, train_size=500, test_size=150) -> dict:
    ...
    for fold_idx in range(3):
        ...
        if model_class_name == "EvolvedStrategyWrapper":
            ...
            def fold_fitness_func(strategy_func):
                try:
                    ...
                    bt = NumbaTurboBacktester()
                    res = bt.run_backtest(df_fold_train, signals)
                    metrics = res['metrics']
        ...
        results = backtester.run_backtest(df_fold_test, predictions)
        sharpe = results['metrics'].get('sharpe_ratio', 0)
...
def run_feature_permutation_importance(model, df_test: pd.DataFrame, features: list) -> dict:
    ...
    base_results = backtester.run_backtest(df_test, base_predictions)
    base_sharpe = base_results['metrics'].get('sharpe_ratio', 0)
    ...
    for feature in tqdm(features, desc="  -> Permuting Features"):
        ...
        permuted_results = backtester.run_backtest(df_permuted, permuted_predictions)
        permuted_sharpe = permuted_results['metrics'].get('sharpe_ratio', 0)
    ...
    if is_gp_strategy:
        ...
        check_results = backtester.run_backtest(df_check, check_pred)
        permuted_sharpe = check_results['metrics'].get('sharpe_ratio', 0)
...
def run_cost_stress_test(model, df_test: pd.DataFrame, features: list) -> dict:
    ...
    try:
        ...
        results = backtester.run_backtest(df_test, predictions)
...
def run_noise_injection_test(model, df_test: pd.DataFrame, features: list) -> dict:
    ...
    try:
        ...
        results = backtester.run_backtest(df_noisy, predictions)
...
def run_full_gauntlet(model, df_processed: pd.DataFrame, asset_symbol: str, reporter=None, df_train_insample: pd.DataFrame = None, model_instance_id: str = None):
    """
    ...
    try:
        ...
        wfa_results = run_walk_forward_analysis(model_class_name, model_params, df_train_main.copy(), features, train_size=1000, test_size=250)
